...
if ($arr_post_data['id']) {
		...
		redirect('baang/profile/'.$arr_post_data['id']);	
...
$orderno.=form_dropdown('order_no', $get_order_dropdown,"",$js);
		if (strpos(strtolower($category_name),'fish') !== false) {
			...
			$fishunit='<div class="product-price">Unit: <i class="fa fa-lg fa-inr"></i> <span>'.$get_product_details['unit'].'</span></div>';
						...
						<div class="col-md-4">
							...
							'.$fishunit.'
								...
								<div class="quantity clearfix" style="margin-bottom:10px">
                                    <label style="margin-bottom:0px">Select Quantity</label>';
...
<li style="margin-bottom:12px"><i class="icon-play" style="font-size:12px"></i>We have received a payment of <b>Rs. '.$amount.'</b>. Your order will soon be shipped.</li>';
		 ...
		 $this->session->set_userdata('paymentsuccessurl', 'Congratulations, welcome to baang family with every order/subscription you help a needy');
		// redirect('/');
